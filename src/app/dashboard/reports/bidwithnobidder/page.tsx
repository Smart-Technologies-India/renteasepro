...
import bidEnding from "@/action/report/bidending";
import bidWithNoBidder from "@/action/report/bidwithnobidder";
...
const ShopBidHistoryView = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const bids = await bidWithNoBidder({});
    ...
    <>
      <div className="p-6">
        <div className="flex gap-4 items-center">
          <BackButton />
          <p className="text-xl text-gray-600">Bid with no Bidders</p>
              ...
              {bids.map((bid, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="text-right">
                    <Link
                      href={`/dashboard/bids/userbidinfo/${encryptURLData(bid.id.toString())}`}
