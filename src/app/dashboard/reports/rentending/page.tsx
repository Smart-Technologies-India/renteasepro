...
const ShopBidHistoryView = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const bids = await getRentEnding({});
    ...
    <>
      <div className="p-6">
        <div className="flex gap-4 items-center">
          <BackButton />
          <p className="text-xl text-gray-600">Rent Ending</p>
            ...
            <TableBody>
              {bids.map((bid, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>&#8377;{bid.rent_amount}</TableCell>
                  <TableCell>
                    {formateDate(new Date(bid.rent_start_date))}
                  </TableCell>
                  <TableCell>
                    {formateDate(new Date(bid.rent_end_date))}
                  </TableCell>
                  <TableCell>{bid.status}</TableCell>
                  <TableCell className="text-right">
                    <Link
                      href={`/dashboard/shops/details/${encryptURLData(bid.shop.id.toString())}`}
