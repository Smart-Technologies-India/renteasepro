...
const CollectDeposite = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (daily_rent_transact.status) {
        const rentresponse = await GetDailyRentById({
          id: parseInt(daily_rent_transact.data?.rentId.toString() ?? "0"),
        });
  ...
  const paydeposit = async () => {
    ...
    const payrent_response = await PayDeposit({
      id: id,
...
export default CollectDeposite;
